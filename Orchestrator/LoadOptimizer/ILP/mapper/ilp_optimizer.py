...
class ILPOptimizer(Mapper):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # ADIL: Initialize the LpProblem of pulp ILP solver model with the
        # given name and sense (either min or max for the single objective).
        self._model = Model(name=name, sense={'max': _MAX, 'min': _MIN}[sense])
        solver = kwargs.pop('solver', None)
        
        # ADIL: If the solver is not mentioned, use the default solver CBC is selected. 
    ...
    def _set_objective(self) -> None:
        ...
        # Added by UmU.
        elif self._criteria == "EIMP":
            model.sense = _MIN
            
            energy_consumption_adjustment = 0.01
            migration_cost_adjustment = 0.1
            pend_penalty_adjustment = 1.1
            migration_cost_adjustment = 1
            
            # energy_consumption_adjustment = 1
            # migration_cost_adjustment = 1
            # pend_penalty_adjustment = 1
            
            
            # Minimize the energy consumption.
            energy_consumption = energy_consumption_adjustment * sum_(self._host_caps[host_id].base_energy_consumption * self._y[host_id] 
                                      + self._host_caps[host_id].per_core_energy_consumption * self._vm_reqs[vm_id].cpu_ratio * self._x_next[vm_id, host_id] 
                                      for vm_id, host_id in self._x_next)

            # Minimize the interference.    
            # Calculate interference cost as average of normalized host energy consumption
            interference_cost = (-1) * sum_(
                sum_(self._host_caps[host_id].per_core_energy_consumption * self._vm_reqs[vm_id].cpu_ratio * self._x_next[vm_id, host_id] 
                    for vm_id, host_id in self._x_next if host_id == h_id) / self._host_caps[h_id].max_energy_consumption
                for h_id in self._host_caps) / len(self._host_caps)
            

            # Minimize the number of migrations.
            # Calculate migration cost as sum of differences between current and next allocations
            # Created proxy variables for absolute values
            # migration_diffs = {}
            # for vm_id, host_id in set(self._x_next.keys()) | self._x_prev:
            #     var_name = f'migration_diff_{vm_id}_{host_id}'
            #     migration_diffs[(vm_id, host_id)] = Var(name=var_name, lowBound=0)
            #     # Adding constraints to model absolute value:
            #     # migration_diff >= prev - next
            #     # migration_diff >= next - prev
            #     prev_val = float((vm_id, host_id) in self._x_prev)
            #     next_val = self._x_next.get((vm_id, host_id), 0)
            #     self._model += migration_diffs[(vm_id, host_id)] >= prev_val - next_val
            #     self._model += migration_diffs[(vm_id, host_id)] >= next_val - prev_val
            # # Sum up all the proxy variables
            # migration_cost = migration_cost_adjustment * sum_(migration_diffs.values())
            
            # Minimize the number of migrations.
            migration_cost = migration_cost_adjustment * sum_(self._n_migr.values())

            # Minimize the number of pending VMs.
            pend_penalty = pend_penalty_adjustment * sum_(self._x_pend.values())
            
            # Set the objective function.
            model +=  energy_consumption + interference_cost + migration_cost + pend_penalty
        else:
            raise NotImplementedError()
